...
@section('customer_main')
    ...
    <!-- <input type="text" name="car_name" placeholder="Search Here">
...
<div class="row row-cols-1 row-cols-md-4 g-4"style="width: 80rem;">
    ...
    <div class="card" style="width: 100rem;">
        <img src="{{asset('pro_images/'.$C->car_pic)}}" class="card-img-top"height="180" >
        <div class="card-body">
          <a href=""><h5>{{ $C->car_name }} {{ $C->car_model }}</h5></a>
          <h4>Provider: {{ $C->car_owner_name }}</h4>
          <h6> {{ $C->car_details }}</h6>
          <a href="view_car_list_details/{{encrypt($C->id)}}" class="btn btn-primary">Details</a>

          @if ($C->rent_status==1 )
            <h6>Unavailable</h6>

            @else
            <a href="{{route('rent_view',['id'=>$C->id])}}" class="btn btn-primary">Rent</a>
            @endif
